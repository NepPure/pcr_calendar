            ...
            for hdday, hddic in month_data["day"].items():
                try:
                    hdstarttime = datetime.strptime(
                        f"{month_data['year']}-{month_data['month']}-{hdday} 05:00",
                        r"%Y-%m-%d %H:%M",
                    )
                except:
                    # 哔哩哔哩你家2022年2月有29号啊？
                ...
                for hdtype, hdcontent in hddic.items():
                    ...
                    if hdtype == "qdhd":
                        ...
                        hdendtime = datetime.strptime(
                            f"{month_data['year']}-{month_data['month']}-{hdday} 04:59",
                            r"%Y-%m-%d %H:%M",
                        )
                    else:
                        hdendtime = datetime.strptime(
                            f"{month_data['year']}-{month_data['month']}-{hdday} 23:59",
                            r"%Y-%m-%d %H:%M",
                        )

                    hdcontent_list = hdcontent_rex.findall(hdcontent)
...
async def load_event_tw():
    ...
    if data:
        ...
        for item in data:
            ...
            end_time = datetime.strptime(item["end_time"], r"%Y/%m/%d %H:%M")
            event = {
...
async def load_event_jp():
    data = await query_data(
        url="https://cdn.jsdelivr.net/gh/pcrbot/calendar-updater-action@gh-pages/jp.json"
    )
    if data:
        event_data["jp"] = []
        for item in data:
...
async def get_events(server, offset, days):
    ...
    # 按type从大到小 按剩余天数从小到大
    events.sort(
        key=lambda item: item["type"] * 100 -
        item["left_days"] - item["start_days"],
        reverse=True,
    )
    return events
